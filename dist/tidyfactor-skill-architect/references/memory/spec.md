...
14. **Dual Discovery & Active Context Synthesis (Anti-Bot & Interactive Options).** Every skill workflow MUST execute a dual context engine: (a) **Automated Exploration First**: Proactively inspect local workspace files, `BRAND-GUIDELINES.md`, `.tidyfactor/*` briefs, codebase structure, open tabs, and config before prompting the user—never ask for information already present on disk; (b) **Structured Interactive Options**: When input or strategic branching is required, present concise, structured multiple-choice options (A/B/C/D) or interactive prompts instead of passive monologues or text dumps; (c) **Zero Robotic Preamble**: Strictly forbid bot persona greetings, self-introductions, lecturing, or textbook dumps; (d) **Context Synthesis**: Synthesize discovered facts and confirmed choices into a persistent baseline (`.tidyfactor/<skill>-brief.md`), locking downstream commands to the confirmed context.
